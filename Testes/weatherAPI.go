...
func main() {
	...
	//consuming API
	res, err := http.Get("http://api.openweathermap.org/data/2.5/weather?q=Porto&APPID=ce303237ff11019fe89330479659adec")
	if err != nil {
		fmt.Print(err)
	}
	...
	label5 := canvas.NewText(fmt.Sprintf("Country %s", weather.Name), color.Black)
	...
	w.ShowAndRun() //show and run the program
}
